...
export class CustomerDeliveryPolicyError
  extends Error {
...
export type CustomerDeliveryLocale =
  | 'vi'
  | 'lo'
  | 'en';

export interface CustomerDeliveryRequestInput {
  salesOrderId: string;
  orderNumber: string;
  channel: CustomerDeliveryChannel;
  locale: CustomerDeliveryLocale;
...
export interface NormalizedCustomerDeliveryRequest {
  salesOrderId: string;
  orderNumber: string;
  channel: CustomerDeliveryChannel;
  locale: CustomerDeliveryLocale;
...
const normalizeUuid = (
  ...
  if (
    ...
    throw new CustomerDeliveryPolicyError(
...
export const normalizeCustomerDeliveryRequest = (
  ...
  if (
    ...
    input.expectedAssetCount < 1 ||
    input.expectedAssetCount > 20
  ) {
    throw new CustomerDeliveryPolicyError(
      'expectedAssetCount must be 1..20',
    );
  }
  if (
...
export const bindDeliveryRequestId = (
  requestId: string,
  payload:
    NormalizedCustomerDeliveryRequest[
      'outboxPayload'
