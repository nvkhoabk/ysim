...
describe('VS-R1-027 customer delivery scheduler adapter', () => {
  ...
  it('does not schedule before explicit start', () => { vi.useFakeTimers(); const runBatch = vi.fn(); new CustomerDeliverySchedulerService({ runBatch } as never, options(true) as never); vi.advanceTimersByTime(5000); expect(runBatch).not.toHaveBeenCalled(); });
  it('runs with configured batch size after the interval', async () => { vi.useFakeTimers(); const runBatch = vi.fn().mockResolvedValue(summary); const service = new CustomerDeliverySchedulerService({ runBatch } as never, options(true) as never); service.start(); await vi.advanceTimersByTimeAsync(1000); expect(runBatch).toHaveBeenCalledWith(7); service.onModuleDestroy(); });
  ...
  it('stops future ticks on shutdown', async () => { vi.useFakeTimers(); const runBatch = vi.fn().mockResolvedValue(summary); const service = new CustomerDeliverySchedulerService({ runBatch } as never, options(true) as never); service.start(); service.onModuleDestroy(); await vi.advanceTimersByTimeAsync(5000); expect(runBatch).not.toHaveBeenCalled(); });
  it('continues scheduling after a rejected tick', async () => { vi.useFakeTimers(); const runBatch = vi.fn().mockRejectedValueOnce(new Error('provider details')).mockResolvedValue(summary); const service = new CustomerDeliverySchedulerService({ runBatch } as never, options(true) as never); service.start(); await vi.advanceTimersByTimeAsync(2000); expect(runBatch).toHaveBeenCalledTimes(2); service.onModuleDestroy(); });
});
