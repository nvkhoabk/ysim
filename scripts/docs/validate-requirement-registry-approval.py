...
EXPECTED_APPROVAL_HISTORY_SHA256 = "fd6a2bb0801c283849da5adb77f5bff287cf6e435e3574b50531910dfb5c3328"
...
def validate_candidate() -> list[str]:
    errors: list[str] = []
    ...
    if not APPROVAL_PATH.is_file() or sha256(APPROVAL_PATH) != EXPECTED_APPROVAL_HISTORY_SHA256:
        errors.append("FC1 approval history or FC2 pending approval envelope bytes differ")
        approval_lines: list[str] = []
    else:
        approval_lines = APPROVAL_PATH.read_text(encoding="utf-8").splitlines()
    for expected_line in EXPECTED_HISTORY_LINES + EXPECTED_FC2_PENDING_LINES + EXPECTED_GAP_LINES:
        expected_count = 2 if expected_line in {
            "- Candidate ID: `V23-REQ-REGISTRY-FC1`",
            "- Revision status: `SUPERSEDED`",
        } else 1
...
def main() -> int:
    ...
    parser.add_argument("--candidate", action="store_true", help="validate FC2 as pending human approval")
    ...
    if args.candidate:
        ...
        return 0
    print("FAIL — FC2_PENDING_HUMAN_APPROVAL", file=sys.stderr)
    print("- V23-REQ-REGISTRY-FC2 is a valid candidate but has no effective human approval.", file=sys.stderr)
    return 1
