...
import { CustomerDeliveryRequestService } from '../../apps/api/dist/modules/delivery/application/customer-delivery-request.service.js';
import { CustomerDeliveryRepository } from '../../apps/api/dist/modules/delivery/infrastructure/customer-delivery.repository.js';
import { EsimAssetIngestionService } from '../../apps/api/dist/modules/fulfillment/application/esim-asset-ingestion.service.js';
import { buildEsimAssetAad } from '../../apps/api/dist/modules/fulfillment/domain/esim-asset-policy.js';
import { EsimAssetCrypto } from '../../apps/api/dist/modules/fulfillment/infrastructure/esim-asset.crypto.js';
import { EsimAssetRepository } from '../../apps/api/dist/modules/fulfillment/infrastructure/esim-asset.repository.js';
...
const runId =
  `ysim-vs-r1-020-${randomBytes(5).toString('hex')}`;
...
const orderAccessToken =
  'runtime-order-access-token-020-'.padEnd(48, 'x');
...
try {
  ...
  const eventId =
    'gpay:event:intent-runtime-020-0001';
  ...
  const createdAssets =
    await assetIngestion.ingest(
      assetBatch,
    );
  const replayedAssets =
    await assetIngestion.ingest(
      assetBatch,
    );
  ...
  if (
    createdAssets.kind !==
      'CREATED' ||
    createdAssets.assetCount !== 2 ||
    replayedAssets.kind !==
      'REPLAY' ||
    replayedAssets.assetCount !== 2 ||
    JSON.stringify(
      [...createdAssets.assetIds].sort(),
    ) !==
      JSON.stringify(
        [...replayedAssets.assetIds].sort(),
      )
  ) {
    throw new Error(
      'Unexpected eSIM Asset ingestion result',
    );
  }
  ...
  const serializedAssetRows =
    JSON.stringify(assetRows);

  if (
    assetRows.length !== 2 ||
    prohibitedColumns.some(
      (column) =>
        assetColumns.includes(column),
    ) ||
    rawSensitiveValues.some(
      (value) =>
        serializedAssetRows.includes(
          value,
        ),
    ) ||
    assetRows.some(
    ...
    throw new Error(
      'Unexpected encrypted eSIM Asset persistence',
    );
  }

  for (const row of assetRows) {
    const sourceAsset =
      assetBatch.esims.find(
        (item) =>
          item.detailId ===
          Number(
    ...
    if (!sourceAsset) {
      throw new Error(
        'Cannot resolve runtime eSIM source',
      );
    }
    ...
    if (
      ...
      decrypted.qrCode !==
        sourceAsset.qrCode ||
      decrypted.shortLink !==
        sourceAsset.shortLink ||
      decrypted.phoneNumber !==
        sourceAsset.phoneNumber
    ) {
      throw new Error(
        'Encrypted eSIM Asset round trip mismatch',
      );
    }
  }

  const postAssetOrder = (
    await pool.query(
      `SELECT
         status,
         payment_status,
  ...
  if (
    postAssetOrder.status !==
      'CONFIRMED' ||
    postAssetOrder.payment_status !==
      'PAID' ||
    postAssetOrder
      .fulfillment_status !==
      'UNFULFILLED' ||
    postAssetProcurement.status !==
      'SUBMITTED'
  ) {
    throw new Error(
      'eSIM Asset persistence changed upstream status',
    );
  }
  ...
  const deliveryService =
    new CustomerDeliveryRequestService(
      new CustomerDeliveryRepository(
        database,
      ),
    );

  const deliveryRequestedAt =
    new Date(
      submissionAt.getTime() +
      3_000,
    ).toISOString();
  ...
  if (
    createdDelivery.kind !==
      'CREATED' ||
    createdDelivery.assetCount !== 2 ||
    createdDelivery.requestStatus !==
      'READY' ||
    createdDelivery.outboxStatus !==
      'PENDING' ||
    replayedDelivery.kind !==
      'REPLAY' ||
    replayedDelivery.deliveryRequestId !==
      createdDelivery.deliveryRequestId ||
    replayedDelivery.outboxId !==
      createdDelivery.outboxId ||
    replayedDelivery.assetCount !== 2 ||
    conflictingDelivery.kind !==
      'CONFLICT'
  ) {
    throw new Error(
      'Unexpected Customer Delivery request lifecycle',
    );
  }
  ...
  if (
    ...
    deliveryRequests[0].id !==
      createdDelivery.deliveryRequestId ||
    deliveryRequests[0].sales_order_id !==
      ids.order ||
    deliveryRequests[0].order_number !==
      runtimeOrderNumber ||
    deliveryRequests[0].channel !==
      'EMAIL' ||
    deliveryRequests[0].locale !==
      'vi' ||
    deliveryRequests[0].delivery_version !==
      1 ||
    deliveryRequests[0]
      .expected_asset_count !== 2 ||
    deliveryRequests[0].status !==
      'READY' ||
    !/^[0-9a-f]{64}$/u.test(
      deliveryRequests[0]
        .asset_set_hash,
    ...
    deliveryOutbox[0]
      ...
      createdDelivery
        .deliveryRequestId ||
    deliveryOutbox[0].event_type !==
      'delivery.customer_esim_requested.v1' ||
    deliveryOutbox[0].status !==
      'PENDING' ||
    deliveryOutbox[0].attempt_count !==
      0 ||
    deliveryOutbox[0].published_at !==
      null ||
    deliveryOutbox[0].last_error !==
      null ||
    deliveryOutbox[0].payload
      .schemaVersion !==
    ...
    deliverySensitiveValues.some(
      (value) =>
        serializedDeliveryState
          .includes(value),
    )
  ) {
    throw new Error(
      'Unexpected safe Customer Delivery persistence',
    );
  }

  const postDeliveryOrder = (
    await pool.query(
      `SELECT
         status,
         payment_status,
  ...
  if (
    postDeliveryOrder.status !==
      'CONFIRMED' ||
    postDeliveryOrder.payment_status !==
      'PAID' ||
    postDeliveryOrder
      .fulfillment_status !==
      'UNFULFILLED'
    ...
    throw new Error(
      'Customer Delivery request changed Sales Order status',
    );
  }
  ...
  Object.assign(evidence.assertions, {
    ...
    customer_delivery_automatic_worker_started: false,
  });
  evidence.database = {
    ...
    esim_asset_plaintext_columns:
      assetColumns.filter(
        (column) =>
          prohibitedColumns.includes(
            column,
          ),
      ).length,
    esim_asset_encryption_algorithm:
      assetRows[0]
        .encrypted_payload.algorithm,
    esim_asset_key_id:
      assetRows[0]
        .encrypted_payload.keyId,
    esim_asset_replay_kind:
      replayedAssets.kind,
    customer_delivery_requests:
      deliveryRequests.length,
    customer_delivery_request_status:
      deliveryRequests[0].status,
    customer_delivery_asset_references:
      deliveryAssets.length,
    customer_delivery_outbox:
      deliveryOutbox.length,
    customer_delivery_outbox_status:
      deliveryOutbox[0].status,
    customer_delivery_outbox_asset_count:
      deliveryOutbox[0]
        .payload.assetCount,
    customer_delivery_replay_kind:
      replayedDelivery.kind,
    customer_delivery_conflict_kind:
      conflictingDelivery.kind,
  };
