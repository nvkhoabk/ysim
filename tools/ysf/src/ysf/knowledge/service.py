...
def read_json(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise KnowledgeBuildError(f"Missing input file: {path}")

    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise KnowledgeBuildError(f"Invalid JSON in {path}: {exc}") from exc

    if not isinstance(data, dict):
        raise KnowledgeBuildError(f"Expected JSON object in {path}")
...
def build_knowledge(
    ...
    source = build_document_index(repository_root)
    raw_documents = source.get("documents")

    if not isinstance(raw_documents, list):
        raise KnowledgeBuildError("documents.json has no documents array")
    ...
    relationships = sorted(
        (capability_relationships + integration_relationships),
        key=lambda item: item.id,
    )

    document_sets = build_document_set_catalog(documents)
    ...
    document_sets_payload = {
        "schemaVersion": "1.1",
        "generatedAt": generated_at,
        "documentSetCount": len(document_sets),
    ...
    counts_by_set = Counter(document.document_set for document in documents)

    summary_payload = {
        "schemaVersion": "1.1",
        "generatedAt": generated_at,
        "status": "PASS",
        "sourceIndex": "in-memory:docs/**/*.md",
        "documentCount": len(documents),
        "documentSetCount": len(counts_by_set),
        "capabilityCount": len(capabilities),
        "integrationCount": len(integrations),
        "relationshipCount": len(relationships),
        "nullGovernedDocumentCodes": 0,
        "documentsBySet": dict(sorted(counts_by_set.items())),
        "notes": [
            ("Capability and integration matches are deterministic heuristics."),
            ("Heuristic relationships require human or AI review."),
            ("docs/ remains the authoritative source."),
        ],
    }

    outputs = {
        "normalized/documents.json": (normalized_payload),
        "catalog/documents.json": (documents_payload),
        "catalog/document-sets.json": (document_sets_payload),
        "catalog/capabilities.json": (capabilities_payload),
        "catalog/integrations.json": (integrations_payload),
        "catalog/relationships.json": (relationships_payload),
        "catalog/knowledge-graph.json": (graph_payload),
        "catalog/summary.json": (summary_payload),
    }

    output_paths: list[str] = []

    for relative_path, payload in outputs.items():
        output_path = knowledge_root / relative_path
        ...
        output_paths.append(output_path.relative_to(repository_root).as_posix())
